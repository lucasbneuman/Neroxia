...
async def update_deal(deal_id: int, deal_update: DealUpdate, db: AsyncSession = Depends(get_db)):
    """Update a deal."""
    try:
...
async def update_deal_stage(deal_id: int, stage_update: DealStageUpdate, db: AsyncSession = Depends(get_db)):
    """Update deal stage."""
    try:
...
async def mark_deal_won(deal_id: int, db: AsyncSession = Depends(get_db)):
    """Mark deal as won."""
    try:
...
async def mark_deal_lost(deal_id: int, reason: str = Query(...), db: AsyncSession = Depends(get_db)):
    """Mark deal as lost."""
    try:
        updated_deal = await crud.mark_deal_lost(db, deal_id, reason)
        if not updated_deal:
            raise HTTPException(status_code=404, detail="Deal not found")
        return updated_deal
    except Exception as e:
...
async def get_user_notes(user_id: int, db: AsyncSession = Depends(get_db)):
...
async def create_note(user_id: int, note: NoteCreate, db: AsyncSession = Depends(get_db)):
    """Create a new note."""
    try:
...
async def add_tag_to_user(user_id: int, tag_id: int, db: AsyncSession = Depends(get_db)):
    """Assign tag to user."""
    try:
...
async def remove_tag_from_user(user_id: int, tag_id: int, db: AsyncSession = Depends(get_db)):
